...
**Question: Explain how the range for specifying arrays works differently in R and Python**
...
Log into the virtual machine with `vagrant ssh`.  This brings you into the server.  Then just install Pandas via `pip install pandas`.  This will download and install the Pandas and any other requirements. 
...
You can download some excellent ipython notebooks [here](http://pandas.pydata.org/pandas-docs/version/0.16.2/tutorials.html#pandas-cookbook) that cover some of the basics of data frames.
...
8. Specify the R code to do each of the following: (a.) a random number between 1 and 8.  (b.) a random integer between 1 and 10. (c.) a random set of 2 integers 1-10 that are not the same, (d.) a random number with  
...
10. Is there a difference between the data stored via the alldata and the alldata2 array in the above example?  Why?

11. Is there a difference with the somedata versions of ages/grades and the all data version of ages/grades?

12. Challenge Question:  Create a function that accepts 2 value (n, m) and returns an N x M matrix with random numbers with a mean of 20 and a standard deviation of 5.  

*Python*

13. Write Python code to set a variable  n equal to 144 and then take the square route of it.  

14. Using Python, write a set of commands that swaps a set of variables such that A is equal to B and B is equal to A. 
...
16. Create a function which returns an array of 2 different strings from a character array.  *Hint. https://docs.python.org/2/library/random.html* 
