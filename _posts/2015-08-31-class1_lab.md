...
### Assignment 1 
It may be that at times there could be an error or enhancement necessary for this class repository.  I'd love your help by fixing these. These fixes are uploaded via a pull request.  I'd like you help in providing a short description of yourself as well as a picture.  

The description should be uploaded to the /people/RPI/2015 directory.  You will find in here a basic template I've used called `kuruzovich_jason.yaml.  [Yaml](http://yaml.org) is an human readable way of storing and encoding data that is increasingly used by websites and other programs.  
(1) Copy the `blank_student.yaml` and change it to `last_first.yaml` where *last* and *first* refer to your names. 
(2) Fill in the associated information in  `last_first.yaml`.
(3) Add a profile picture to the photos directory.
(4) Commit the changes changes using GitHub desktop.  
(5) Sync the repository.
(6) Issue a pull request to the main branch.

You should also test that both the R Studio Server and the Python development environment are working properly.
