...
```{r}
#Lets get the structure of the columns.
names(titantic)
#We can see that survived=1, sex=4, and age =5
tnew1<-titantic[,c(1,4,5)] 
tnew2<-titantic[,c('survived', 'sex', 'age')] 
tnew3<-subset(titantic, select=c('survived', 'sex', 'age') )
```

2R. Drop the *ticket* and the cabin and create a new dataframe with the results.

```{r}
#Let's get the structure of the columns.

#We can see that ticket=8, sex=4, and cabin =10
tnew4<-titantic[,c(-8,-10)] 
...
rows<-round(.8*n,0)
#We can see that ticket=8, sex=4, and cabin =10
tnew6<-titantic[1:rows,] 
tnew7<-titantic[(rows+1):n,] 
...
tnew8<-titantic[select1,] 
tnew9<-titantic[select2,] 
#or
tnew10<-titantic[r[1:halfn],] 
tnew11<-titantic[r[(halfn+1):n],] 
...
#Males/Females Array via a vector listing the desired rows
malesarray<-which(titantic$sex=='male')
femalesarray<-which(titantic$sex=='female')
males2<-titantic[ femalesarray, ] 
...
```{r}
titanic <- rbind(titanic,titantic_morerows)
```
...
```{r}
#This Works. 
all_kuruzovich <- merge(titanic,titantic_morecolumns,by="Name",all.x=TRUE, all.y=TRUE)  #y is df3 (like full outer join)
```
